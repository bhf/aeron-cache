...
class CacheEntryRemovedResultTest {
    ...
    public static final int CACHE_ENTRY_REMOVED_TID = 1;
    ...
    void shouldDecodePassResultToCallbackHandler() {
        ...
        var timeStamp = 1L;
        var length = 1024;
        when(schemaDetailsProvider.getCacheEntryRemovedId()).thenReturn(CACHE_ENTRY_REMOVED_TID);

        requestBuffer.putShort(TID_INDEX, (short) CACHE_ENTRY_REMOVED_TID);
        ...
        // Assert
        verify(
                cacheResponseDecoder, times(CACHE_ENTRY_REMOVED_TID)).decodeCacheEntryRemoved(
                any(DirectBuffer.class), anyInt(), any(RemoveCacheEntryResult.class)
        );

        verify(callbackHandler, times(CACHE_ENTRY_REMOVED_TID))
