...
class CacheEntryResultTest {

    public static final int CACHE_ENTRY_TID = 1;
    ...
    @Test
    @DisplayName("Should decode cache entry result and pass to results callback handler")
    @HappyPath
    void shouldDecodePassResultToCallbackHandler() {
        ...
        var timeStamp = 1L;
        var length = 1024;
        when(schemaDetailsProvider.getCacheEntryResultId()).thenReturn(CACHE_ENTRY_TID);

        requestBuffer.putShort(TID_INDEX, (short) CACHE_ENTRY_TID);
        ...
        // Assert
        verify(
                cacheResponseDecoder, times(CACHE_ENTRY_TID)).decodeGetCacheEntryResult(
                any(DirectBuffer.class), anyInt(), any(GetCacheEntryResult.class)
        );

        verify(callbackHandler, times(CACHE_ENTRY_TID))
