...
class CacheAllStatsResultTest {

    public static final int CACHE_ALL_STATS_TID = 1;
    ...
    void shouldDecodePassResultToCallbackHandler() {
        ...
        var timeStamp = 1L;
        var length = 1024;
        when(schemaDetailsProvider.getAllCacheStatsResultId()).thenReturn(CACHE_ALL_STATS_TID);

        requestBuffer.putShort(TID_INDEX, (short) CACHE_ALL_STATS_TID);
        ...
        // Assert
        verify(
                cacheResponseDecoder, times(CACHE_ALL_STATS_TID)).decodeAllCacheStatsResult(
                any(DirectBuffer.class), anyInt(), any(CacheStatsResult.class)
        );

        verify(callbackHandler, times(CACHE_ALL_STATS_TID))
