...
class CacheDeletedResultTest {
    ...
    public static final int CACHE_DELETED_TID = 1;
    ...
    void shouldDecodePassResultToCallbackHandler() {
        ...
        var timeStamp = 1L;
        var length = 1024;
        when(schemaDetailsProvider.getCacheDeletedId()).thenReturn(CACHE_DELETED_TID);

        requestBuffer.putShort(TID_INDEX, (short) CACHE_DELETED_TID);
        ...
        // Assert
        verify(
                cacheResponseDecoder, times(CACHE_DELETED_TID)).decodeCacheDeleted(
                any(DirectBuffer.class), anyInt(), any(DeleteCacheResult.class)
        );

        verify(callbackHandler, times(CACHE_DELETED_TID))
