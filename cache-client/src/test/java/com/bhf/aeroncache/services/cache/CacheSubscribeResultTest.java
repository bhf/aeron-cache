...
class CacheSubscribeResultTest {
    ...
    public static final int CACHE_SUBSCRIBE_TID = 1;
    ...
    void shouldDecodePassResultToCallbackHandler() {
        ...
        var timeStamp = 1L;
        var length = 1024;
        when(schemaDetailsProvider.getCacheSubscriptionResponseId()).thenReturn(CACHE_SUBSCRIBE_TID);

        requestBuffer.putShort(TID_INDEX, (short) CACHE_SUBSCRIBE_TID);
        ...
        // Assert
        verify(
                cacheResponseDecoder, times(CACHE_SUBSCRIBE_TID)).decodeCacheSubscribeResult(
                any(DirectBuffer.class), anyInt(), any(CacheSubscriptionResult.class)
        );

        verify(callbackHandler, times(CACHE_SUBSCRIBE_TID))
