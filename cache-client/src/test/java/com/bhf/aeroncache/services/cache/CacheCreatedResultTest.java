...
import com.bhf.aeroncache.messages.CacheCreatedEncoder;
...
class CacheCreatedResultTest {
    ...
    void shouldDecodePassResultToCallbackHandler() {
        ...
        var length = 1024;
        cacheCreatedEncoder.wrapAndApplyHeader(requestBuffer, 0, headerEncoder);
        ...
        try (MockedStatic<CacheResponseDecoder> encoder = Mockito.mockStatic(CacheResponseDecoder.class)) {
            ...
            // Assert
            encoder.verify(() ->
                            CacheResponseDecoder.decodeCacheCreated(
                                    any(CreateCacheResult.class),
                                    any(CacheCreatedDecoder.class),
