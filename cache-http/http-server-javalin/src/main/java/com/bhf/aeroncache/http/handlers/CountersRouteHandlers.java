...
import com.bhf.aeroncache.http.requests.DecrementCounterRequest;
...
public class CountersRouteHandlers extends AbstractRouteHandlers<ReusableLong, Long> {
    ...
    public void handleDecrementItemRequest(Context ctx) {
        try {
            var cacheId = ctx.pathParam("cacheId");
            var request = ctx.bodyAsClass(DecrementCounterRequest.class);
            log.info("Got decrement counter request on cacheId {}, key {}, amount {}", cacheId, request.key(), request.amount());

            var requestId = getRequestId(ctx);
            CompletableFuture<DecrementCounterResponse> future = new CompletableFuture<>();
            Consumer<DecrementCounterResult<ReusableString, ReusableString>> consumer = c -> {
                log.info("Decrement counter response from cluster on cacheId {}, key {}, value {}", c.getCacheId(),
                        c.getKey(), c.getCounterValue());
                var noCache = c.getStatus() == CacheOperationStatus.UNKNOWN_CACHE;
                var response = noCache ?
                        new DecrementCounterResponse("0", "NA", 0L, c.getStatus()) :
                        new DecrementCounterResponse(c.getCacheId().value().toString(), c.getKey().value().toString(),
                                c.getCounterValue(), c.getStatus());
                future.complete(response);
            };
            long ttl = 0;
            CompletableFuture.runAsync(() -> publisher.decrementCounter(requestId, cacheId,
                    request.key(), request.amount(), ttl, consumer));
