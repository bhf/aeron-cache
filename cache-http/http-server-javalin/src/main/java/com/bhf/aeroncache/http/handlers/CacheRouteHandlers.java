...
@Log4j2
public class CacheRouteHandlers extends AbstractRouteHandlers<ReusableString, String> {

    static final Pattern specialCharacters = Pattern.compile("[$&+,:;=\\\\?@#|/'<>.^*()%!]");
    static final Set<String> invalidCacheNames = Set.of("bulkops", "timed");

    public CacheRouteHandlers(ObservingCacheRequestPublisher<ReusableString, ReusableString, ReusableString, String, String, String> cachePublisher) {
        super(cachePublisher);
    }
    ...
    public void handleGetItemRequest(Context ctx) {
        try {
            ...
            var consumer = HTTPConsumerUtils.getGetCacheEntryResultConsumer(future);
