...
import com.bhf.aeroncache.services.cache.impl.AgentCacheRequestPublisher;
...
/**
 * Wired up HTTP Application that uses the approach of encoding
 * HTTP requests into a {@link ManyToOneRingBuffer} which is
 * processed by an {@link AgentRunner} which handles the actual
 * encoding into SBE message and sending too Aeron Cache.
 */
@Log4j2
public class HttpRBApplication {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            CacheRequestPublisher rbPublisher = new AgentCacheRequestPublisher(rb);
    ...
    private static Javalin startHTTPServer() {
        ...
        return Javalin.create(config)
                ...
                .post(API_PREFIX, HttpRBApplication::handleCreateCacheRequest)
                .get(API_PREFIX + "<cacheId>/<key>", HttpRBApplication::handleGetItemRequest)
                .get(API_PREFIX + "<cacheId>", HttpRBApplication::handleGetCacheRequest)
                .post(API_PREFIX + "<cacheId>", HttpRBApplication::handlePutItemRequest)
                .delete(API_PREFIX + "<cacheId>/<key>", HttpRBApplication::handleDeleteItemRequest)
                .delete(API_PREFIX + "<cacheId>", HttpRBApplication::handleDeleteCacheRequest)
                .patch(API_PREFIX + "<cacheId>", HttpRBApplication::handleClearCacheRequest)
                .get("/api/v1/caches", HttpRBApplication::handleGetCachesRequest)
                .get("/api/v1/stats", HttpRBApplication::handleGetStatsRequest)
                .get(LIVENESS, HttpRBApplication::handleGetLiveness)
                .get(READINESS, HttpRBApplication::handleGetReadiness)
                .get("/prometheus", ctx -> ctx.contentType(PROMO_MICROMETER_CONTENT_TYPE).result(registry.scrape()))
                .post("baselinePost", HttpRBApplication::postActionBaseline)
                .get("baselineGet", HttpRBApplication::getActionBaseline)
                .start(PORT);
    }
