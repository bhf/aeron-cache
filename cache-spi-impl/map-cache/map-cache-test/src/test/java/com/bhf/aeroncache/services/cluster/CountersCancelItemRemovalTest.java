...
public class CountersCancelItemRemovalTest extends AbstractCancelItemRemovalTest<ReusableString, ReusableString, ReusableLong, ReusableString> {
    ...
    public CountersCancelItemRemovalTest() {
        super(cacheManagerFactory_);
    }

    @Override
    protected CancelItemRemovalResult<ReusableString, ReusableString> createResult() {
        return new CancelItemRemovalResult<>(SupplierUtils.stringSupplier.get(), SupplierUtils.stringSupplier.get());
    }
    ...
    @Override
    public int encodeCancelItemRemoval(String requestId, ReusableString cacheId, ReusableString key, MutableDirectBuffer buffer) {
        return cacheRequestEncoder.encodeCancelItemRemoval(requestId, cacheId, key, buffer);
    }

    @Override
    public void decodeItemRemovalCancelled(DirectBuffer buffer, int offset, CancelItemRemovalResult<ReusableString, ReusableString> result) {
        cacheResponseDecoder.decodeItemRemovalCancelled(buffer, offset, result);
    }
