...
public class DecrementCounterTest {
    ...
    private DecrementCounterResult<ReusableString, ReusableString> result;
    ...
    void setup() {
        ...
        result = new DecrementCounterResult<>(SupplierUtils.stringSupplier.get(), SupplierUtils.stringSupplier.get());
    }

    @Test
    @DisplayName("Should decrement counter on a known cache")
    void shouldDecrementCounter() {
        ...
        var length = cacheRequestEncoder.encodeDecrementCounterRequest(requestId, cacheId, counterId, decrementAmount, 0, requestBuffer);
        sut.onSessionMessage(session, System.currentTimeMillis(), requestBuffer, 0, length, header);

        cacheResponseDecoder.decodeDecrementCounterResponse(responseBuffer, 0, result);
        ...
        assertEquals(initialValue - decrementAmount, result.getCounterValue());
    ...
    @Test
    @DisplayName("Should return unknown cache when decrementing on non-existent cache")
    void shouldReturnUnknownCacheWhenCacheDoesNotExist() {
        ...
        var requestId = UUID.randomUUID().toString();
        var length = cacheRequestEncoder.encodeDecrementCounterRequest(requestId, cacheId, counterId, 10L, 0, requestBuffer);
        sut.onSessionMessage(session, System.currentTimeMillis(), requestBuffer, 0, length, header);
