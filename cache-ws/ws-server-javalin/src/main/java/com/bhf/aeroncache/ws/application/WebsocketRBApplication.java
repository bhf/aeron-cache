...
import com.bhf.aeroncache.ws.services.subscriptions.CacheSubscriptionRBService;
...
@Log4j2
public class WebsocketRBApplication {
    ...
    private static CacheSubscriptionRBService subscriptionService;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            subscriptionService = new CacheSubscriptionRBService(rbPublisher);
    ...
    private static Javalin startHTTPServer() {
        ...
        return Javalin.create(config)
                ...
                .ws(API_PREFIX + "/{cacheId}", WebsocketRBApplication::handleSingleCacheWs)
                .ws(MULTI_SUB_API_PREFIX + "/{cacheIds}", WebsocketRBApplication::handleMultiCacheWs)
                .get(LIVENESS, WebsocketRBApplication::handleGetLiveness)
                .get(READINESS, WebsocketRBApplication::handleGetReadiness)
    ...
    private static void handleSingleCacheWs(WsConfig wsConfig) {
        wsConfig.onConnect(WebsocketRBApplication::onSingleCacheConnect);
        wsConfig.onClose(WebsocketRBApplication::onWsClose);
        wsConfig.onError(WebsocketRBApplication::onWsError);
        wsConfig.onMessage(WebsocketRBApplication::onWsMessage);
    }
    ...
    private static void handleMultiCacheWs(WsConfig wsConfig) {
        wsConfig.onConnect(WebsocketRBApplication::onMultiCacheConnect);
        wsConfig.onClose(WebsocketRBApplication::onWsClose);
        wsConfig.onError(WebsocketRBApplication::onWsError);
        wsConfig.onMessage(WebsocketRBApplication::onWsMessage);
    }
