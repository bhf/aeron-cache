...
// end::client[]
{
    ...
    private void sendMessageToCluster(final AeronCluster aeronCluster)
    {
        ...
        while (!Thread.currentThread().isInterrupted())
        {
            ...
            {
                nextBidDeadlineMs = currentTimeMs + ThreadLocalRandom.current().nextInt(100);
    ...
    private long sendBid(final AeronCluster aeronCluster)
    {
        ...
        idleStrategy.reset();
        while (aeronCluster.offer(actionBidBuffer, 0, createEncoder.encodedLength()+headerEncoder.encodedLength()) < 0)
        {
            idleStrategy.idle(aeronCluster.pollEgress());
        }
