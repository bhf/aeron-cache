...
import { deleteCacheRequest } from "@/lib/actions";
...
function alertOnErrorSending() {
  const props = {
    title: "Error sending delete request",
...
function toastFailure(reason: string) {
  const props = {
    title: "Error",
    description: "Failed to delete cache: " + reason,
    actionLabel: "OK",
  };
...
export function DeleteCache(props: DeleteCacheProps) {
  ...
  return (
    <ConfirmingDialog
      title={"Deleting Cache: " + props.cacheId}
      message={"Are you sure you want to delete this cache?"}
      cancelText={"Cancel"}
      actionText={"Continue"}
      action={sendDeleteCacheRequest}
      buttonText={"Delete"}
    />
  );
}