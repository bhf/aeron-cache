...
export default function AddItemRequest(props: AddItemProps) {
    ...
    async function toasterAlert() {
        if (!hasToastShown) {
            ...
            let props = {title: "Error sending add item request", description: "Couldn't add item", actionLabel: "OK"};
            toast(props.title, {
    ...
    async function toastSuccess() {
        if (!hasToastShown) {
            await setHasToastShown(true);
            await setFormSubmitted(false);
            let props = {title: "Success", description: "Added item", actionLabel: "OK"};
