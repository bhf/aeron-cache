...
public class ClearCacheBenchmark {

    /**
     * The cache is populated once with {@link #cacheItemsSize} entries during setup. Clearing is
     * destructive, so only the first invocation of a measurement iteration empties a fully populated
     * cache; subsequent invocations clear an already-empty cache. The measured cost is therefore
     * dominated by the request decode, handler dispatch, subscription notification and response
     * encode rather than by the map teardown itself.
     */
    ...
    @Benchmark
    public void clearCache(Blackhole bh) {
        seq++;
