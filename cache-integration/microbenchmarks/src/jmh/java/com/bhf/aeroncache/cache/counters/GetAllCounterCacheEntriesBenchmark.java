...
public class GetAllCounterCacheEntriesBenchmark {
    ...
    private GetAllCacheEntriesResult<ReusableString, ReusableString, ReusableLong> getAllCacheEntriesResult;
    ...
    public void setup() {
        ...
        getAllCacheEntriesResult = new GetAllCacheEntriesResult<>(SupplierUtils.stringSupplier.get());
    ...
    @Benchmark
    public void getAllCounterCacheEntries(Blackhole bh) {
        seq++;
        var requestId = "req-" + seq;

        var length = requestEncoder.encodeGetCacheEntries(
                requestId, reusableCacheId, requestBuffer);

        sut.onSessionMessage(session, ++tsCounter, requestBuffer, 0, length, header);
        responseDecoder.decodeAllCacheEntriesResult(responseBuffer, 0, getAllCacheEntriesResult);

        bh.consume(getAllCacheEntriesResult);
    }
