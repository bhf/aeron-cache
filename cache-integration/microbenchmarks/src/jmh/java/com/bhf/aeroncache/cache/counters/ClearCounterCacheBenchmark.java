...
public class ClearCounterCacheBenchmark {

    /**
     * The counter cache is populated once with {@link #cacheItemsSize} entries during setup. Clearing
     * is destructive, so only the first invocation of a measurement iteration empties a fully
     * populated cache; subsequent invocations clear an already-empty cache. The measured cost is
     * therefore dominated by the request decode, handler dispatch, subscription notification and
     * response encode rather than by the map teardown itself.
     */
    ...
    private ClearCacheResult<ReusableString> clearCacheResult;
    ...
    public void setup() {
        ...
        clearCacheResult = new ClearCacheResult<>(SupplierUtils.stringSupplier.get());
    ...
    @Benchmark
    public void clearCounterCache(Blackhole bh) {
        seq++;
        var requestId = "req-" + seq;

        var length = requestEncoder.encodeClearCache(
                requestId, reusableCacheId, requestBuffer);

        sut.onSessionMessage(session, ++tsCounter, requestBuffer, 0, length, header);
        responseDecoder.decodeCacheCleared(responseBuffer, 0, clearCacheResult);

        bh.consume(clearCacheResult);
    }
