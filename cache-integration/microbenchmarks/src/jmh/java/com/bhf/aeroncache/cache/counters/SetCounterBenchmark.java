...
public class SetCounterBenchmark {
    ...
    private SetCounterResult<ReusableString, ReusableString> setCounterResult;
    ...
    public void setup() {
        ...
        setCounterResult = new SetCounterResult<>(SupplierUtils.stringSupplier.get(), SupplierUtils.stringSupplier.get());
    ...
    @Benchmark
    public void setCounter(Blackhole bh) {
        seq++;
        var requestId = "req-" + seq;

        var length = requestEncoder.encodeSetCounterRequest(
                requestId, reusableCacheId, reusableKey, seq, 0L, requestBuffer);

        sut.onSessionMessage(session, ++tsCounter, requestBuffer, 0, length, header);
        responseDecoder.decodeSetCounterResponse(responseBuffer, 0, setCounterResult);

        bh.consume(setCounterResult);
    }
