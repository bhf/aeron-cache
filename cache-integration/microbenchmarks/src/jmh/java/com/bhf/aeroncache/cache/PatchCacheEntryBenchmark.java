...
public class PatchCacheEntryBenchmark {
    ...
    private PatchValueResult<ReusableString, ReusableString, ReusableString> patchValueResult;
    ...
    public void setup() {
        ...
        patchValueResult = new PatchValueResult<>(SupplierUtils.stringSupplier.get(), SupplierUtils.stringSupplier.get(), SupplierUtils.stringSupplier.get());
    ...
    @Benchmark
    public void patchCacheEntry(Blackhole bh) {
        seq++;
        ...
        reusableValue.clear();
        reusableValue.copyFrom("{\"v\":" + seq + "}");

        var requestId = "req-" + seq;

        var length = requestEncoder.encodePatchValue(
                requestId, reusableCacheId, reusableKey, reusableValue, requestBuffer);

        sut.onSessionMessage(session, ++tsCounter, requestBuffer, 0, length, header);
        responseDecoder.decodePatchValueResult(responseBuffer, 0, patchValueResult);

        bh.consume(patchValueResult);
    }
