...
public class IncrementCounterBenchmark {
    ...
    private IncrementCounterResult<ReusableString, ReusableString> incrementCounterResult;
    ...
    public void setup() {
        ...
        incrementCounterResult = new IncrementCounterResult<>(SupplierUtils.stringSupplier.get(), SupplierUtils.stringSupplier.get());
    ...
    @Benchmark
    public void incrementCounter(Blackhole bh) {
        seq++;
        var requestId = "req-" + seq;

        var length = requestEncoder.encodeIncrementCounterRequest(
                requestId, reusableCacheId, reusableKey, 1L, 0L, requestBuffer);

        sut.onSessionMessage(session, ++tsCounter, requestBuffer, 0, length, header);
        responseDecoder.decodeIncrementCounterResponse(responseBuffer, 0, incrementCounterResult);

        bh.consume(incrementCounterResult);
    }
