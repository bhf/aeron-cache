...
public abstract class AbstractMultiStreamBulkOpsCountersTests {
    ...
    private static final Integer KNOWN_VALUE = 3;

    private final StreamingHelper[] streamingHelpers;
    private TestEndpointsProvider bulkOpsEndpoints;

    protected AbstractMultiStreamBulkOpsCountersTests(TestEndpointsProvider endpointsProvider, StreamingHelper... streamingHelpers) {
    ...
    void shouldGetStreamingUpdatesWhenAddingMultipleItemsForSubscribedCache(BackendTestResource backend) {
        ...
        int i=0;
        for (; i < numItems; i++) {
            operations.add(new CacheOperationRequest(BulkOperationType.ADD_COUNTER, 0,KNOWN_VALUE,  "req-" + i, KNOWN_CACHE_ID, KNOWN_KEY + "-" + i, ""));
        }
        operations.add(new CacheOperationRequest(BulkOperationType.ADD_COUNTER, 0,KNOWN_VALUE,  "req-" + i, NON_SUBSCRIBED_CACHE_ID, KNOWN_KEY + "-" + i, ""));
    ...
    void shouldGetStreamingUpdatesOnMixedBulkOps(BackendTestResource backend) {
        ...
        var operations = List.of(
                new CacheOperationRequest(BulkOperationType.ADD_COUNTER, 0, KNOWN_VALUE, UUID.randomUUID().toString(), KNOWN_CACHE_ID, testKey, null),
                new CacheOperationRequest(BulkOperationType.ADD_COUNTER, 0, KNOWN_VALUE, UUID.randomUUID().toString(), KNOWN_CACHE_ID, testKey, null),
                new CacheOperationRequest(BulkOperationType.REMOVE_COUNTER, 0, 0, UUID.randomUUID().toString(), KNOWN_CACHE_ID, testKey, null),
                new CacheOperationRequest(BulkOperationType.CLEAR_COUNTER_CACHE, 0, 0, UUID.randomUUID().toString(), KNOWN_CACHE_ID, null, null)
        );
        ...
        for (var streamingSourceEventsFuture : perStreamingSourceEvents) {
            ...
            MatcherAssert.assertThat(secondAddEvent.itemValue(), Matchers.is(KNOWN_VALUE));
    ...
    void shouldGetRemoveEventWhenItemWithTtlExpires(BackendTestResource backend) {
        ...
        var operations = List.of(
                new CacheOperationRequest(BulkOperationType.ADD_COUNTER, ttlMs, KNOWN_VALUE,  UUID.randomUUID().toString(), KNOWN_CACHE_ID, ttlKey, null)
        );
