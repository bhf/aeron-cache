...
public abstract class AbstractMultiStreamDecrementCounterTests {
    ...
    protected AbstractMultiStreamDecrementCounterTests(TestEndpointsProvider endpointsProvider, StreamingHelper... streamingHelpers) {
    ...
    @Test
    @DisplayName("Should get a streaming update when decrementing a counter in a known cache")
    @HappyPath
    protected void shouldGetStreamingUpdateWhenDecrementingCounter(BackendTestResource backend) {
        // Arrange - seed the counter key before subscribing so the seed event is not captured
        CacheTestUtils.addItem(getKnownCacheId(), KNOWN_KEY, 10, backend, countersEndpoints);
        var perStreamingSourceEvents = StreamingHelperUtil.getPerStreamEvents(streamingHelpers, backend, getKnownCacheId(), 1);

        // Act
        CacheTestUtils.decrementCounter(getKnownCacheId(), KNOWN_KEY, 4, backend, countersEndpoints);
        ...
        for (var streamingSourceEventsFuture : perStreamingSourceEvents) {
            ...
            MatcherAssert.assertThat(updateEvent.itemValue(), Matchers.is(6));
        }
    }

    @Test
    @DisplayName("Should get ordered streaming updates when accumulating multiple decrements")
    @HappyPath
    protected void shouldGetStreamingUpdatesWhenAccumulatingDecrements(BackendTestResource backend) {
        // Arrange - seed the counter key before subscribing so the seed event is not captured
        CacheTestUtils.addItem(getKnownCacheId(), ACCUMULATING_KEY, 10, backend, countersEndpoints);
        var perStreamingSourceEvents = StreamingHelperUtil.getPerStreamEvents(streamingHelpers, backend, getKnownCacheId(), 2);

        // Act
        CacheTestUtils.decrementCounter(getKnownCacheId(), ACCUMULATING_KEY, 5, backend, countersEndpoints);
        CacheTestUtils.decrementCounter(getKnownCacheId(), ACCUMULATING_KEY, 3, backend, countersEndpoints);
        ...
        for (var streamingSourceEventsFuture : perStreamingSourceEvents) {
            ...
            MatcherAssert.assertThat(secondUpdate.itemValue(), Matchers.is(2));
        }
    }
